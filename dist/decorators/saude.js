...
class SaudeDeduction extends deduction_decorator_js_1.DeductionDecorator {
    applyDeduction(out, input) {
        const bruto = input.deducoes?.gastosSaude ?? 0;
        const teto = tax_table_provider_js_1.TaxTableProvider.getInstance().getForYear(input.ano).limites.saudeMax;
...
exports.SaudeDeduction = SaudeDeduction;
